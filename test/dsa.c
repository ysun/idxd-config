...
void build_sgl_list(struct task* tsk, int opcode, uint8_t type_size, uint32_t elem_cnt)
{
	uint32_t i = 0;

	for (i = 0; i < tsk->sgl_config.sgl_size; i++) {
			...
			if (0 == stage) {
				...
				for (int i = 0; i < element_count; i++) {
					...
					case DSA_COMPUTE_ADD:
						...
						break;
					case DSA_COMPUTE_AND:
						p_dst[i] = (uint64_t)p_dst[i] & (uint32_t)p_src[i];
						break;
					case DSA_COMPUTE_OR:
						p_dst[i] = (uint64_t)p_dst[i] ^ (uint32_t)p_src[i];
						break;
	...
	for (sgl_idx = 0; sgl_idx < tsk->sgl_config.sgl_size; sgl_idx++) {
		...
		for (i = 0; i < element_count; i++)
			do_elem_compute(dst_verify_addr, odata_type_size, sgl_src, idata_type_size,
				tsk->elemwise_cofig.compute_type, tsk->elemwise_cofig.compute_flags, element_count, i);
	}
}
